...
```{r}
# counts how many times the ibis_coverage_start_date is earlier than the coverage_start_date in  report_full 
...
```{r}
patients |> distinct(pid) # Checking the reports rows of unique data
n_distinct(patients$pid)
```
